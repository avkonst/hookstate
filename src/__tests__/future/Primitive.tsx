import { useState, createState, $get, $set, $keys } from '../../';
...
test('primitive: should not rerender unused', async () => {
    ...
    expect(renderTimes).toStrictEqual(1);
    expect(result.current[$get]).toStrictEqual(1);
    expect(() => result.current['non-existing']).toThrow('StateLink is used incorrectly. Attempted \'set\' at \'/\'. Hint: target value is not an object to contain properties');
    expect(() => result.current[0]).toThrow('StateLink is used incorrectly. Attempted \'set\' at \'/\'. Hint: target value is not an object to contain properties');
});
